        ...
        <form onSubmit={onSubmitHandler} className="flex-1 bg-blue-50/50 text-gray-600 h-full overflow-scroll">
            <div className='bg-white w-full max-w-3xl p-4 md:p-10 sm:m-10 shadow rounded'>
                ...
                <div className='max-w-lg h-74 pb-16 sm:pb-10 pt-2 relative'>
                    ...
                    {loading && (<div className='absolute right-0 top-0 left-0 bottom-0 left-0 flex items-center justify-center'>
                 ...
                 <select onChange={e => setCategory(e.target.value)} value={category} name="category" className='mt-2 px-3 py-2 border text-gray-500 border-gray-300 outline-none rounded'>
                    ...
                    {blogCategories.map((item, index)=> {return <option key={index} value={item}>{item}</option>})}
                 </select>
                 <div className='flex gap-2 mt-4'>
                    <p>Publish Now</p>
                    <input type="checkbox" checked={isPublished} className='sacle-125 cursor-pointer' onChange={e => setIsPublished(e.target.checked)}/>
                 </div>
